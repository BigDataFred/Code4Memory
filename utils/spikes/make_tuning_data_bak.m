...
if nargin ==0
    sf.p2d = '/home/rouxf/MATLAB/toolboxes/Wave_Clus/UCLA_data/';
    sf.p2sd = [sf.p2d,'sort/5/'];
    make_figure = 'yes';
    chan_number = [ 4 ];
    logf = [];
...
try
    ...
    filename = [sf.p2d,'A',num2str(chan_number),'.Ncs'];
...
%%
dataset = [sf.p2d,'A',num2str(chan_number),'.Ncs'];
fn = dir(dataset);
if isempty(fn)
...
for it = 1:length(spdat.useNegative)
    ...
    for jt = 1:length(sel_idx)
        x(jt) = ( spdat.newTimestampsNegative(jt) - double(hdr.FirstTimeStamp))./hdr.TimeStampPerSample + 1;
    end;
    ...
    [CSC_preproc2{it}] = launch_LFP_preproc(dataset,trl);
    ...
    [fft2] = ft_freqanalysis(cfg,CSC_preproc2);
    ...
    cfg.preproc.lpfilttype = 'but';
    cfg.preproc.padtype = 'mirror';
    cfg.preproc.padding = tlck.time(end)*5;
    
    [tlck2] = ft_timelockanalysis(cfg,CSC_preproc2);
    clear CSC_preproc2;
...
if strcmp(make_figure,'yes')
    ...
    subplot(223);
    hold on;
    plot([min(tlck.time) max(tlck2.time)],[0 0],'r--');
    plot(tlck2.time,tlck2.avg);
    subplot(224);
    hold on;
    plot([min(fft2.freq) max(fft2.freq)],[0 0],'r--');
    plot(fft2.freq,20*log10(fft2.powspctrm));
    %%
