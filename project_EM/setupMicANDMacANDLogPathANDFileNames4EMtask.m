function [p2SpkDat,spkFiles,p2micDat,micFiles,p2macDat,macFile,macDat,ixIEDdat,logDat] = setupMicANDMacANDLogPathANDFileNames4EMtask(pID,expMode,expSel,sesh,seshSel,BFlabel,BFlabel2,bfSel,bfSel2,MWlabel,mwSel,micDatMode)

%% load the Logfile
p2logDat = ['/media/rouxf/rds-share/iEEG_DATA/MICRO/',pID,'/',expMode{expSel},'/',sesh{seshSel},'/log_dat/'];
logFile = dir([p2logDat,'*_LogFile_EMtask_LogDat.mat']);
...
spkDat = [pID,'_TS',num2str(seshSel),'_spike_data_CSC_',BFlabel2{bfSel2},MWlabel{mwSel},'_',sesh{seshSel},'_stimlocked.mat'];
...
if micDatMode == 2
    micDat = [pID,'_',expMode{expSel},'_CSC_',BFlabel{bfSel},'_',sesh{seshSel},'_preprocANDrejectedIED_stimlocked.mat'];
else
    micDat = [pID,'_TS',num2str(seshSel),'_lfp_data_CSC_',BFlabel{bfSel},'*_',sesh{seshSel},'_stimlocked.mat'];
end;
...
ixIEDdat.samp = [];
ixIEDdat.selIx = [];
IEDfile = dir([p2micDat,pID,'_',expMode{expSel},'_CSC_',BFlabel{bfSel},'_',sesh{seshSel},'_preprocANDrejectedIED_stimlocked.mat']);
if ~isempty(IEDfile)
    IEDfile = dir([p2micDat,pID,'_',expMode{expSel},'_CSC_',BFlabel{bfSel},'_',sesh{seshSel},'_preprocANDrejectedIED_stimlocked.mat']);
elseif isempty(bfSel)
    for it  = 1:length(BFlabel)
        IEDfile = dir([p2micDat,pID,'_',expMode{expSel},'_CSC_',BFlabel{it},'_',sesh{seshSel},'_preprocANDrejectedIED_stimlocked.mat']);
        if ~isempty(IEDfile)
